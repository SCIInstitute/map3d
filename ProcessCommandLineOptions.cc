...
int ProcessCommandLineOptions(Global_Input & g)
{
  ...
  // set window widths
  //   override defaults
  if (g.borderWidth > 0 && g.titleHeight > 0) {
   map3d_info.globalBorderWidth = g.borderWidth;
    map3d_info.globalTitleHeight = g.titleHeight;
  }
  //   if borderless, set normal borders to 0, but keep globals
  if (g.qnoborders) {
    map3d_info.borderWidth = map3d_info.titleHeight = 0;
  }
  else {
    map3d_info.borderWidth = map3d_info.globalBorderWidth;
    map3d_info.titleHeight = map3d_info.globalTitleHeight;
  }

  if (g.imagefile)
    strcpy(map3d_info.imagefile, g.imagefile);
  map3d_info.posx = map3d_info.borderWidth + 1;
  map3d_info.posy = map3d_info.titleHeight + map3d_info.borderWidth + 1;
  int loop, length;
  Mesh_List *meshes = new Mesh_List;
  //MainWindow *mainpriv = 0;
  Mesh_Info *curmesh = 0;

  int width, height;
  /* when we create window, the size we input 
     specify the client region size, so the real
     size is clientRegion+borderWidth
   */

  map3d_info.screenHeight = gdk_screen_height();
  map3d_info.screenWidth = gdk_screen_width();
  ...
  if (g.qnoborders) {
    ...
    map3d_info.parentid = masterWindow->winid;
  ...
  for (win = 0; win < MAX_SURFS; win++) {
    map3d_info.geomwins[win] = new GeomWindow;
    gtk_widget_show(map3d_info.geomwins[win]->drawarea);
    map3d_info.legendwins[win] = new LegendWindow;
  ...
  for (loop = 0; loop < length && loop < MAX_SURFS; loop++) {
    ...
    if (returnedMeshes.size() == 0)
      ...
      if (!geompriv || !g.qsinglewin || surf->newwindow) {
        width = surf->winxmax - surf->winxmin - 2 * map3d_info.borderWidth;
        height = surf->winymax - surf->winymin - VERTICAL;
        geompriv = GeomWindow::GeomWindowCreate(width, height, surf->winxmin, surf->winymin, 256, 256);
  ...
  for (loop = 0; loop < length; loop++) {
	...
	pInfo->rms = 1;
      ...
      pickPriv->setPosAndShow();
      curmesh->pickstacktop++;
      curmesh->pickstack[curmesh->pickstacktop] = pInfo;

    }
  }
  if (map3d_info.posy > 100) {
    map3d_info.borderless_xmax = map3d_info.screenWidth;
    map3d_info.borderless_xmin = 0;
  }
  if (map3d_info.wrap_flag > 0) {
    map3d_info.borderless_ymin = 0;
    map3d_info.borderless_ymax = map3d_info.screenHeight;
  }
...
//#if 0 //for new fid dialog
          ...
          switch(mesh->fidConts.back()->datatype){
            case 0:
              gdk_color_parse("slategray",&mesh->fidConts.back()->fidcolor);
              break;
            case 1:
              gdk_color_parse("slategray",&mesh->fidConts.back()->fidcolor);
              break;
            case 2:
              gdk_color_parse("magenta",&mesh->fidConts.back()->fidcolor);
              break;
            case 3:
              gdk_color_parse("slategray",&mesh->fidConts.back()->fidcolor);
              break;
            ...
            case 6:
              ...
              break;
            case 7:
              gdk_color_parse("deepskyblue2",&mesh->fidConts.back()->fidcolor);
              break;
            case 8:
              gdk_color_parse("slategray",&mesh->fidConts.back()->fidcolor);
              break;
            case 9:
              ...
              break;
            case 10:
              gdk_color_parse("red",&mesh->fidConts.back()->fidcolor);
              break;
            case 11:
              gdk_color_parse("slategray",&mesh->fidConts.back()->fidcolor);
              break;
            case 12:
              ...
              break;
            case 13:
              gdk_color_parse("green",&mesh->fidConts.back()->fidcolor);
              break;
            case 14:
              gdk_color_parse("yellow",&mesh->fidConts.back()->fidcolor);
              break;
            case 15:
              gdk_color_parse("slategray",&mesh->fidConts.back()->fidcolor);
              break;
            case 16:
              gdk_color_parse("blue",&mesh->fidConts.back()->fidcolor);
              break;              
          }
