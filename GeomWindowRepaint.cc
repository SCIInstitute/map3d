...
void GeomWindowRepaint(GtkWidget *widget, GdkEvent *, gpointer data)
{
  ...
  // have been done yet...
  if (!priv->initialized)
    GeomWindowReshape(widget, NULL, data);
  ...
  //printf("fog1 = %f\n",priv->fog1);
  //printf("fog2 = %f\n",priv->fog2);
  glFogf(GL_FOG_START, priv->l2norm * priv->fog1);
  glFogf(GL_FOG_END, priv->l2norm * priv->fog2);
  ...
  // draw rotated light
  HMatrix mNow;
  Ball_Value(&priv->light_pos, mNow);
  ...
  for (; loop < length; loop++) {
    ...
    if (curmesh->drawmesh && curmesh->geom->points[curmesh->geom->geom_index]) {
      ...
      if (loop == priv->secondarysurf && length > 1)
        DrawMesh(curmesh, 1);
      else
        DrawMesh(curmesh, 0);
  ...
  for (; loop < length; loop++) {
    ...
    if (curmesh->axes && (priv->all_axes || start == loop)) {
      DrawAxes(curmesh);
    }
    ...
    if (curmesh->geom->landmarks && curmesh->landmarkdraw.qshowlmark)
      DrawLandMarks(curmesh->geom->landmarks, &curmesh->landmarkdraw, priv);
  ...
  if (priv->showinfotext && !priv->clip->back_enabled && !priv->clip->front_enabled) {
    DrawInfo(priv);
  }
  
  
  /* draw lock icons */
  if (priv->showlocks && !priv->clip->back_enabled && !priv->clip->front_enabled) {
    if (map3d_info.lockgeneral != LOCK_OFF)
      DrawLockSymbol(priv, 0, map3d_info.lockgeneral == LOCK_FULL);
    if (map3d_info.lockrotate != LOCK_OFF)
      DrawLockSymbol(priv, 1, map3d_info.lockrotate == LOCK_FULL);
    if (map3d_info.lockframes != LOCK_OFF)
      DrawLockSymbol(priv, 2, map3d_info.lockframes == LOCK_FULL);
  }
...
void DrawFidCont(Mesh_Info * curmesh, Contour_Info *cont)
{
  ...
  if(curcont->fidContSize > 0){
    for (loop2 = 0; loop2 < length; loop2++) {
      ...
      glColor3d(curcont->fidcolor.red/65535.0,curcont->fidcolor.green/65535.0,curcont->fidcolor.blue/65535.0);
...
void Transform(Mesh_Info * curmesh, float factor)
{
  ...
  glTranslatef(0, 0, priv->l2norm * (factor - 2));
  
  /* draw clipping planes (if enabled) */
  glPushMatrix();
  glMultMatrixf((float *)cNow);
  glTranslatef(priv->xcenter, priv->ycenter, priv->zcenter);
...
{
  GeomWindow *priv = curmesh->gpriv;  //(GeomWindow*) GetCurrentWindow();
  ...
  if (curmesh->mark_all_sphere || curmesh->mark_extrema_sphere || curmesh->mark_ts_sphere || curmesh->mark_lead_sphere || 
    ...
    for (loop = 0; loop < length; loop++) {
      ...
      if ((curmesh->num_selected_pts > 0 && curmesh->selected_pts[0] == loop) ||
        ...
        glPointSize(priv->height / 200 * curmesh->mark_triangulate_size);
      ...
      // pick node
        ...
        glPointSize(priv->height / 200 * curmesh->mark_ts_size);
      ...
      // extrema node
        ...
        glPointSize(priv->height / 200 * curmesh->mark_extrema_size);
      }
      ...
      // 'all' node
        ...
        glPointSize(priv->height / 200 * curmesh->mark_all_size);
      ...
      sphere_size = sphere_size*priv->l2norm/400;
  ...
  if (curmesh->mark_all_number || (curmesh->mark_extrema_number && curmesh->data) ||
    ...
    for (loop = 0; loop < length; loop++) {
      ...
      switch (number) {
      case 1:
        renderString3f(pos[0], pos[1], pos[2], (int)priv->small_font, "%d", loop + 1);
        break;
      case 2:
        if (curgeom->channels[loop]+1 > 0)
          renderString3f(pos[0], pos[1], pos[2], (int)priv->small_font, "%d", curgeom->channels[loop] + 1);
        break;
      case 3:
        if (cursurf && cursurf->potvals && cursurf->potvals[cursurf->framenum][loop] != UNUSED_DATA)
          renderString3f(pos[0], pos[1], pos[2], (int)priv->small_font, "%1.2f",
                          cursurf->potvals[cursurf->framenum][loop]);
        break;
      case 4:
...
{
  int nummesh = priv->meshes.size();
  int surfnum = 0;
  float position[3] = { -1.f, (priv->height - 15.0), 0.f };
  Mesh_Info *dommesh = 0;
  ...
  glPushMatrix();
  glPopMatrix();
  if (nummesh == 1 || (priv->dominantsurf != -1 && nummesh > priv->dominantsurf)) {
    dommesh = nummesh == 1 ? priv->meshes[0] : priv->meshes[priv->dominantsurf];
    char surfstr[24];
    ...
    position[0] = (float)priv->width/2.0 -(((float)getFontWidth((int)priv->large_font)*strlen(surfstr))/2.0);
    position[1] = priv->height - getFontHeight((int)priv->large_font);
    
    renderString3f(position[0], position[1], position[2], (int)priv->large_font, surfstr);
    
    position[1] -= getFontHeight((int)priv->med_font)*.8 ;
    if (dommesh->data){
      char * slash = shorten_filename(dommesh->data->potfilename);
      position[0] = (float)priv->width/2.0 - (((float)getFontWidth((int)priv->med_font)*(strlen(slash)))/2.0);
      renderString3f(position[0], position[1], position[2], (int)priv->med_font, "%s@%d", slash, dommesh->data->seriesnum+1);
    }
    else{
      char * slash = shorten_filename(dommesh->geom->basefilename);
      position[0] = (float)priv->width/2.0 - (((float)getFontWidth((int)priv->med_font)*(strlen(slash)))/2.0);
      renderString3f(position[0], position[1], position[2], (int)priv->med_font, slash);
    }
  }
  else if (nummesh > 1) {
    position[0] = (float)priv->width/2.0 -(((float)getFontWidth((int)priv->large_font)*12)/2.0);
    renderString3f(position[0], position[1], position[2], (int)priv->large_font, "All Surfaces");
  }
}

void DrawLockSymbol(GeomWindow * priv, int which, bool full)
{
  //GeomWindow *priv = (GeomWindow*) GetCurrentWindow();
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  ...
  float aspect = (float)priv->height / priv->width;
...
void DrawAxes(Mesh_Info * mesh)
{
  ...
  //float d = priv->l2norm*mesh->mark_all_size * 25;
  ...
  glVertex3f(priv->xcenter, priv->ycenter, priv->zcenter);
  glVertex3f(delta_x, priv->ycenter, priv->zcenter);
  if(priv->rgb_axes){
    glColor3f(0,255,0);
  }
  else{
  ...
  glVertex3f(priv->xcenter, priv->ycenter, priv->zcenter);
  glVertex3f(priv->xcenter, delta_y, priv->zcenter);
  if(priv->rgb_axes){
    glColor3f(0,0,255);
  }
  else{
  ...
  if (priv->showinfotext) {
    ...
    if (xneg)
      renderString3f(pos[0], pos[1], pos[2], (int)priv->small_font, "-X");
    else
      renderString3f(pos[0], pos[1], pos[2], (int)priv->small_font, "X");
    pos[0] = priv->xcenter;
    pos[1] = delta_y;
    ...
    if (yneg)
      renderString3f(pos[0], pos[1], pos[2], (int)priv->small_font, "-Y");
    else
      renderString3f(pos[0], pos[1], pos[2], (int)priv->small_font, "Y");
    pos[1] = priv->ycenter;
    pos[2] = delta_z;
    ...
    if (zneg)
      renderString3f(pos[0], pos[1], pos[2], (int)priv->small_font, "-Z");
    else
      renderString3f(pos[0], pos[1], pos[2], (int)priv->small_font, "Z");
  }
...
void DrawBGImage(GeomWindow* priv)
{
  if (map3d_info.gi->bgcoords[0] != 0 || map3d_info.gi->bgcoords[3] != 1) {
    ...
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    glTranslatef(0, 0, -2 * priv->l2norm);
    glTranslatef(-priv->xcenter, -priv->ycenter, -priv->zcenter);
 }
  else {
    glMatrixMode(GL_PROJECTION);
