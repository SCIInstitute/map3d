...
#ifdef _WIN32
#pragma warning(disable:4514 4244 4305) /* quiet visual c++ */
#endif
...
Quat Qt_FromBallPoints(HVect from, HVect to, BallType bt)
{
  Quat qu;
  ...
  if (bt == ArcBall) {
    ...
    mag = (float)sqrt((double)qu.x * qu.x + qu.y * qu.y + qu.z * qu.z);
    qu.w = cos(a);
    s = sin(a);
    if (s != 0)
      s /= mag;
