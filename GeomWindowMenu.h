...
//  window_attr_double, window_attr_half,
  window_attr_info_on, window_attr_info_off, window_locks, window_save_dialog, window_save,
...
#define MAP3D_WINDOW_LOCK_TOGGLE(lockname, var) \
  if (!lockname || priv == GetGeomWindow(0)) priv->var = !priv->var; \
  else priv->var = GetGeomWindow(0)->var;  
